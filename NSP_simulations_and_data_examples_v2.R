...
d <- dim(zz.ans_int$intervals)
zz.ans_int_tmp <- zz.ans_int
zz.ans_int_tmp$intervals[2,1] <- 24
zz.ans_int_tmp$intervals[2,2] <- 34

cpts <- rep(0, d[1])
for (i in 1:d[1]) {
	
	tmp <- zz.ans[zz.ans_int_tmp$intervals[i,1]:zz.ans_int_tmp$intervals[i,2]]
	features(not(tmp, contrast="pcwsLinContMean", rand.intervals=F, intervals=all.intervals(length(tmp))), q.max=1)$cpt -> cpt
	cpts[i] <- cpt + zz.ans_int_tmp$intervals[i,1] - 1
