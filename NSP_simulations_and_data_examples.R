...
nsp_coverage_extended <- function(truth, est) {
	...
	list(coverage = res, how.many = d[2])
...
# Note of caution: Examples 1 and 2 below use a version of NSP which uses a random interval sampling mechanism. Code for this implementation is not and will not be provided - we have replaced it with
# a deterministic interval sampling mechanism, for improved reproducibility of results (with the random sampling mechanism reproducibility is still possible via set.seed, but this is frequently forgotten
# and so not very reliable).
# Your results for Examples 1 and 2 may therefore differ *slightly* from the below.
...
z <- rev(cvd[[5]])
filter(z, rep(1, 7)/7, sides=1) -> zz
zz.ans <- 2 * sqrt(zz[8:140] + 3/8)
set.seed(1)
nsp_poly(zz.ans, deg=1) -> zz.ans_int
ts.plot(zz.ans, xlab="Time (days starting from 12th March 2020)", ylab = "")
draw_rects(zz.ans_int, c(0, 70), 20, "grey")


d <- dim(zz.ans_int)
zz.ans_int_tmp <- zz.ans_int
zz.ans_int[1,1] <- 24
zz.ans_int[2,1] <- 34
...
for (i in 1:d[2]) {
	...
	tmp <- zz.ans[zz.ans_int[1,i]:zz.ans_int[2,i]]
	features(not(tmp, contrast="pcwsLinContMean", rand.intervals=F, intervals=all.intervals(length(tmp))), q.max=1)$cpt -> cpt
	cpts[i] <- cpt + zz.ans_int[1,i] - 1
	
}
zz.ans_int <- zz.ans_int_tmp
cpts

abline(v=cpts, col="red")


bss <- bs(1:133, deg=1, knots = cpts, intercept=T)
dd <- lm(zz.ans ~ bss)
lines(dd$fitted, col="green")



# Example 3 - House prices in the London Borough of Newham

read.csv("nth.csv") -> nth
npr <- nth[[7]][seq(from=1, by = 2, length = 125)]

covv <- matrix(1, 124, 2)
covv[,2] <- log(npr[1:124])
resp <- npr[2:125]
nsp_tvreg(resp, covv, 1000, overlap=TRUE)

summary(lm(resp[1:63]~covv[1:63,]-1))
summary(lm(resp[64:124]~covv[64:124,]-1))
...
ts.plot(x.easy, ylab="")
cpt.importance(x.easy.n)
cpt.importance(x.easy.n.1stage)
